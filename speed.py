...
printer('*🌐 Testing speed from %(isp)s...*\n' % speedtest.config['client'], quiet)

if not args.mini:
    printer('📡 Retrieving server list from speedtest.net...', quiet)
    try:
        ...
        printer('Failed to retrieve speedtest server list', error=True)
        raise SpeedtestCLIError(get_exception())
    except InvalidServerIDType:
        raise SpeedtestCLIError(
            '%s is an invalid server type, it must be an integer' 
            % ', '.join('%s' % s for s in args.server)
        )

    if args.server and len(args.server) == 1:
        printer('📍 Fetching details for the selected server...', quiet)
    else:
        printer('⚡ Selecting the best server based on ping...', quiet)
...
if args.download:
    ...
    printer('*📥 Download Speed:* %0.2f M%s/s' %
            ((results.download / 1000.0 / 1000.0) / args.units[1],
             args.units[0]),
            quiet)
else:
    printer('⏭️ Skipping download test', quiet)

if args.upload:
    speedtest.upload()
    printer('*📤 Upload Speed:* %0.2f M%s/s' %
            ((results.upload / 1000.0 / 1000.0) / args.units[1],
             args.units[0]),
            quiet)
    printer("\n-------------------------------------------------\n🚀 *Speed test powered by Speedtest Ookla*")
else:
    printer('⏭️ Skipping upload test', quiet)
